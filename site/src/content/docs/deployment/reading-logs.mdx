...
For Prometheus scraping, **don't parse logs** — Posthorn exposes a native `/metrics` endpoint on the same listener with the same counters. See [Docker → Health checks](/deployment/docker/#health-checks) for wiring it into a Docker healthcheck or Prometheus scrape config.
