...
| 11 | Duplicate-send from worker retries | Idempotency cache (24h TTL) returns byte-identical replay |
| 12 | Replay attack via stale idempotency keys | 24h TTL eviction; in-flight collision returns 409 |
| 13 | Spoofing via leaked API key (per-request `from`) | `from` is endpoint-configured; not overridable per request |
