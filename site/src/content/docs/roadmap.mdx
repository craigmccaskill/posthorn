...
| Batch send API | Deferred without a target version. The named v1.1 audience didn't have a real workload for it; we'll reconsider when an operator surfaces with concrete batch volume that round-trip overhead actually constrains. |
