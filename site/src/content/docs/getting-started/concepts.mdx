...
A **transport** is the egress side — the thing that actually delivers the email. v1.0 ships one transport (Postmark). v1.2 adds Resend, Mailgun, SES, and outbound-SMTP. The `Transport` interface is identical across them, so adding a new one doesn't require changes to the request pipeline.
...
<Aside type="note">
v1.0 has **no persistent queue.** A failed submission that exhausted both retries is gone from in-memory state; only the log line remains. Persistent retry queues land in v2 along with SQLite storage.
</Aside>

## Two deployment shapes, one core

The standalone binary and the Caddy adapter run **the same core gateway code**. The Caddy adapter is a thin (~150 LOC) wrapper that:

1. Registers `http.handlers.posthorn` with Caddy
2. Parses the Caddyfile syntax into the same internal config struct that the TOML loader produces
3. Delegates `ServeHTTP` to the same `gateway.Handler`

If something works in the standalone, it works in the adapter. If a bug exists in the pipeline, it exists in both. Parity tests assert this in CI.
