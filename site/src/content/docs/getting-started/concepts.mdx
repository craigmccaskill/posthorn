...
A **transport** is the egress side — the thing that actually delivers the email. v1.0 ships one transport (Postmark). v1.1+ adds Resend, Mailgun, SES, and outbound-SMTP. The `Transport` interface is identical across them, so adding a new one doesn't require changes to the request pipeline.
