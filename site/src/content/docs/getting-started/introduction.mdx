...
| Capability | Detail |
|---|---|
| HTTP form ingress | `application/x-www-form-urlencoded`, `multipart/form-data`; honeypot + Origin check + rate limit + Cross-Site Request Forgery (CSRF) tokens (opt-in) |
...
| Transports | Postmark, Resend, Mailgun, AWS SES (Signature Version 4 signing), outbound-SMTP relay — five total, all bespoke (no SDK deps) |
