...
This walks through standing up a working contact form on Docker against a real Postmark account. It assumes you've already followed [Installation](/getting-started/installation/) and have a Postmark server token in hand.
...
3. **Write `.env`.**
   ...
   <Aside type="caution">
   The Postmark *server token* is per-server, not per-account. Find it in the Postmark dashboard under your server's API Tokens tab. Never commit this to git.
   </Aside>
