...
A subtle but important invariant: the outbound message's recipients come from the SMTP `RCPT TO` commands, **never** from the MIME `To:`/`Cc:`/`Bcc:` headers. A malicious client that sent:
