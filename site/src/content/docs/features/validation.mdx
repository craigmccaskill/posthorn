...
| Check | When | Notes |
|---|---|---|
| Max field length | v1.2 | Currently bounded only by `max_body_size` |
| Allowed values (enum) | v1.2 | e.g. `category` must be one of `["bug", "feature"]` |
| Regex pattern match | v1.2 | e.g. `phone` must match a pattern |
