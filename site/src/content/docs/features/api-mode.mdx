...
Posthorn tracks failed-auth attempts per client IP. After **10 failed auth attempts from one IP within ~1 minute**, the bucket empties and subsequent failed attempts from that IP return **429 Too Many Requests** instead of 401, with body `too many failed authentication attempts`. The bucket refills at the same rate, so a slow legitimate caller who occasionally typos a key recovers; a brute-force scanner trying thousands of keys per minute hits the lockout within seconds.
