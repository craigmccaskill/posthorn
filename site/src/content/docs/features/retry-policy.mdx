...
- A `submission_failed` log line is written with the underlying error (Posthorn does not emit a separate `timeout_exceeded` event in v1.0 — the timeout surfaces as the cancelled retry's error).
