...
| `password` | yes | — | SMTP AUTH PLAIN password. **Never logged** ([NFR3](https://github.com/craigmccaskill/posthorn/blob/main/spec/02-prd.md)). |
...
| CRLF in `From` / `To` / `Subject` / `Reply-To` ([NFR1](https://github.com/craigmccaskill/posthorn/blob/main/spec/02-prd.md)) | `ErrTerminal` | no (rejected at Posthorn's edge before dialing) |

## Header injection defense

SMTP is the only Posthorn transport that hand-writes the RFC 5322 message envelope (because there's no JSON struct to lean on). The structural NFR1 defense is **pre-write validation**: before the wire conversation starts, Posthorn checks that no submitter-controlled header value contains CR or LF. Any CRLF in `From`/`To`/`Subject`/`Reply-To` is rejected with `ErrTerminal` and the dial never happens.
