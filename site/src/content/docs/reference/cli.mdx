...
The 15-second drain deadline is intentionally longer than the per-request 10-second hard timeout (FR22) so any in-flight retry can complete gracefully. Press Ctrl+C twice in a terminal to force-exit a stuck process.
...
Aliased to `--help` and `-h`. Subcommand-specific help is not separately wired in v1.0 — the top-level `help` lists the flag set for each subcommand.
