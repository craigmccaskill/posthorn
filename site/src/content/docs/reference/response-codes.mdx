...
A `Retry-After` header is **not** emitted — the client gets a plain 429 and is expected to back off. (Adding `Retry-After` derived from token-bucket state is a planned polish item.)
...
<Aside type="caution">
On 502, the submission's form payload is logged at ERROR level (unless `log_failed_submissions = false`). Read your logs to recover the data and send manually if needed. There is no persistent queue; the failed submission only lives in the log. A durable retry queue is planned for v2.
</Aside>
