...
Also returned silently when the **honeypot field is non-empty.** The honeypot path emits the *same* JSON shape — `status: "ok"` plus a fresh `submission_id` — so a bot inspecting the response body cannot tell honeypot rejection apart from a real success.
