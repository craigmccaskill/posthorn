...
<Aside type="note">
**File uploads (multipart with `Content-Type: application/octet-stream` parts) are accepted at the wire level but ignored.** Posthorn does not currently forward attachments to the transport. Attachment support is planned for v2. If your form includes file inputs, the file content is silently dropped — only the text fields make it to the email.
</Aside>
