...
cron -> umami: "POST /api/auth/login"
umami -> cron: "{ token }"

cron -> umami: "GET /api/websites/{id}/stats\n(Authorization: Bearer)"
umami -> cron: "{ pageviews, visitors, … }"

cron -> posthorn: "POST /api/weekly-digest\nIdempotency-Key: 2026-W22"
posthorn -> provider: HTTPS
posthorn -> cron: "{ status: ok, submission_id }"
```
