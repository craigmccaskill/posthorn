...
- A Postmark account with a verified sender signature on your sending domain (SPF + DKIM verified in Postmark; DMARC recommended but not required for a first send — see [DNS](/security/dns/))
...
2. **Write `posthorn.toml`.**
   ...
   <Aside type="tip" title="Hit-reply-to-respond">
   `reply_to_email_field = "email"` is the line that makes "reply" in your mail client go directly to the visitor's address instead of to `noreply@yourdomain.com`. The submitter's email becomes the `Reply-To:` header on every submission that includes a valid email field. Default is the `email_field` value (which itself defaults to `"email"`), so most contact forms get this for free; you only need to set this explicitly when your form's email field has a different name or you want to disable the behavior. Disable by pointing it at a field that doesn't exist (e.g., `reply_to_email_field = "_disabled"`).
   </Aside>
