...
- A Postmark account with a verified sender signature on your sending domain (SPF, DKIM, DMARC set up — see [DNS](/security/dns/))
...
| Real users keep hitting `HTTP 429 rate limit` | Multiple users behind the same NAT, or your reverse proxy isn't forwarding client IPs | Add `trusted_proxies = ["<your-proxy-CIDR>"]` to the endpoint and bump `count` up |
