...
host: Docker host {
  ...
  comentario -> posthorn: "SMTP on posthorn:2525\n(internal Docker network)"
}

provider: Postmark

hugo -> host.posthorn: HTTP form POST
...
- [Self-hosted Ghost via Posthorn](/recipes/ghost/) for the same pattern applied to Ghost's member-mail flows
- [Weekly Umami analytics digest via Posthorn API mode](/recipes/umami/) for the API-mode pattern when the source app doesn't speak SMTP
